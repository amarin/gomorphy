...
### Суффиксы: расширение адресации — ЗАПЛАНИРОВАНО (release-blocking)
...
данных — но значит и не работает вовсе на полном словаре. Нужно решение
по подходу перед релизом 1.0.0 (или явная фиксация в release notes, что
сборка полного OpenCorpora-словаря пока не поддерживается).

**Варианты на обсуждение** (черновой список, не решение):
- Переменный размер индекса суффикса (`uint16` по умолчанию, `uint32`
  когда данных больше) — нужен флаг/версия формата `.dat`, чтобы `Open`
  знал, как читать секцию суффиксов; совместимость со старыми файлами.
- Автоматическое разбиение словаря на несколько DAWG-сегментов
  (шардирование по диапазону суффиксов или лемм) с параллельным
  поиском по сегментам на чтении — сложнее, но не меняет per-сегмент
  формат.
- Что-то ещё — не исследовано; сам факт находки новый, подход не
  прорабатывался.

**Автоматические проверки (тесты, ориентир для будущей реализации)**:
- unit: `ImportFromXML` на синтетическом наборе с >65536 уникальных
  суффиксов — сейчас должен явно ошибаться (regression-тест на текущее
  поведение honest-error, если такого теста ещё нет);
- после фикса — тот же тест должен успешно собрать словарь;
