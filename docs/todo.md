...
## Этап 1. Примитивы формата (internal/format)
...
- unit-тесты roundtrip varint/delta на краевых случаях (0, 1, max u32/u64,
  монотонные/немонотонные последовательности);
- unit-тесты writer→reader: несколько секций, чтение каждой по имени,
  ошибка при битой чексумме/магии/версии;
- `go build ./...`, `go vet ./internal/format`, `go test ./internal/format`.
Ручная проверка: hexdump тестового файла — читаемые magic/каталог.
