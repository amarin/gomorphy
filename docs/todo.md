...
- [x] Реализация поверх postings; значения через арены. Полная грамматическая
...
- [x] Семантика выдачи зафиксирована сверкой с эталоном:
...
Проверка:
- [x] unit-тесты: слова-омонимы («стекла», «пила») → ≥2 леммы; уникальное
  слово → 1; слитый разбор «кота» = NOUN,anim,masc,sing,gent;
- [x] сверка с эталоном: 100 случайных словоформ из 3 065 312 против
  независимого двухпроходного разбора dict.xml (pkg/dictionary/
  reference_integration_test.go): плотные id лемм, полные граммемы и число
  разборов совпали полностью.

## Этап 9. Нечёткий поиск FT6

Инкремент: Fuzzy(word, k).
- [x] DFA Левенштейна (без транспозиций) поверх CSR-trie; отсечение по k;
