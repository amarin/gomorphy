...
### Плотный алфавит в проде: тезисы обсуждения (пауза, не решено) — НЕ СПРОЕКТИРОВАНО

Обсуждалось 2026-09-15/16, сразу после мерджа ветки `dawg-alphabet-harness`
в master. Пользователь взял паузу подумать — **решение не принято**,
ничего из ниже написанного не реализовано. Цель раздела — не потерять
...
- **Не решено, какой вариант брать.** Начинать реализацию рано —
