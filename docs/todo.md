...
5. Продакшн-внедрение плотного 1-байтового DAWG-алфавита — решено
   пользователем сделать до релиза (2026-09-15/16). Harness и находки
   смёржены в master, дизайн внедрения обсуждён, но **не завершён —
   пользователь взял паузу подумать**. См. отдельный раздел ниже
   «Плотный алфавит в проде: тезисы обсуждения (пауза, не решено)».
