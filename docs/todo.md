...
7. **Этап 18 (финализация: документация, тесты, метрики) — НЕ НАЧАТО.**
   CLI-часть уже закрыта пунктом 6. Единственный оставшийся пункт
   между текущим состоянием и релизом — см. ниже.
8. Релиз 1.0.0.
...
### Этап 18. Финализация: документация, тесты — НЕ НАЧАТО

> CLI-редизайн (`cmd/gomorphy`, ранее также `cmd/gomorphy_build`) уже
> выполнен отдельно — см. пункт 6 выше и
> [implementation/pymorphy-source-and-cli.md](implementation/pymorphy-source-and-cli.md).
> Этот этап — оставшаяся документация, тесты, ожидаемые метрики.
