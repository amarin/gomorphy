...
4. Поддержка нескольких параллельно открытых словарей (multi-dict) —
   ...
   Ещё не спроектировано — см. отдельный раздел ниже
   «Multi-dict: известный контекст и открытые вопросы».
...
### Multi-dict: известный контекст и открытые вопросы — НЕ СПРОЕКТИРОВАНО
...
**Открытые вопросы (не решены, требуют отдельного brainstorming перед стартом):**
1. Идентификатор словаря в `Reading`/`LemmaRef` — по аналогии с
   `Shard`, но для внешнего словаря: индекс в пользовательском наборе?
   Код языка? Путь к файлу?
2. API уровня «мультисловарь» — отдельный тип-обёртка (агрегирующий
   `Parse`/`Lemma` по нескольким `*Dictionary`) или набор
   вспомогательных функций поверх существующего `*Dictionary`?
3. Стратегия при пересечении результатов между словарями (слово
   находится в двух словарях сразу) — возвращать оба помеченными по
   словарю? Приоритет по порядку открытия? Явный выбор языка
   пользователем?
4. CLI: несколько `-dict` флагов или список путей; формат вывода —
   как добавить идентификатор словаря к `para#Shard/Para`.
5. Связь с 2-байтовым DAWG-алфавитом (см. п.5 «Путь к версии 1.0.0»
   выше и раздел «Плотный алфавит в проде» ниже): мультисловарь с
   отдельным алфавитом на каждый язык и один
   словарь с широким 2-байтовым алфавитом на объединённый корпус —
   два разных решения одной и той же проблемы (алфавит, не
   умещающийся в 1 байт). Сейчас выбран путь «мультисловарь», а
   2-байтовый алфавит остаётся экспериментальной библиотечной
   возможностью без CLI — при проектировании multi-dict явно
   зафиксировать это соотношение в документации, чтобы не решать
   одну и ту же задачу дважды разными механизмами.
