...
4. Поддержка нескольких параллельно открытых словарей (multi-dict) —
   решено пользователем сделать до релиза, независимо от Этапа 17.
   **ВЫПОЛНЕНО 2026-09-16** — `morphology.MultiDictionary` (Go API,
   без CLI). См. раздел «Multi-dict: известный контекст и открытые
   вопросы» ниже — вопросы решены, там же итог.
5. Продакшн-внедрение плотного 1-байтового DAWG-алфавита — решено
   пользователем сделать до релиза (2026-09-15/16). **Для pymorphy2
   `words.dawg` — ВЫПОЛНЕНО 2026-09-16** (`RecompileDense`,
   `OpenPyMorphyDense`, `DAWG.Walk`, `Dictionary.Alphabet`, см. раздел
   «pymorphy2-рекомпилятор под плотный алфавит» ниже). Открытыми
   остаются: `.dat`-сериализация алфавита, `fuzzy.go`,
   `Prediction`/`Probability` DAWG — если понадобятся до релиза,
...
6. Груминг + редизайн CLI-команд — **ВЫПОЛНЕНО 2026-09-16** — единый
   ...
   `docs/superpowers/plans/2026-09-16-cli-redesign.md` и обновлённый раздел
   «Этап 21» ниже.
7. Этап 18 (финализация: документация, тесты, метрики — CLI-часть уже
   закрыта пунктом 6 выше).
8. Релиз 1.0.0.
...
### Multi-dict: известный контекст и открытые вопросы — РЕШЕНО И РЕАЛИЗОВАНО 2026-09-16
...
это **не** поддерживает сейчас — только «технически не запрещено
открыть N раз `Dictionary` вручную».

**Известный контекст (проверено чтением кода 2026-09-15):**
- Реестра/менеджера нескольких словарей нет. `pkg/morphology/open.go`
  (`Open`, `OpenPyMorphy`, `CompileFromXML*`) — каждый вызов даёт
  независимый `*Dictionary` со своим mmap; ничего не мешает открыть
  несколько, но библиотека их не связывает.
- `Reading` (`parse.go:14-22`) и `LemmaRef` (`lemma.go:4-9`) несут
  только `Shard int` — индекс шарда **внутри одного** словаря, не
  идентификатор словаря/языка. При опросе нескольких словарей
  вызывающему коду придётся вручную оборачивать результаты, чтобы не
  потерять источник.
- CLI (`cmd/gomorphy/main.go:71`) принимает ровно один `-dict <path>`.
  Вывод `lookup` — `Word\tNormal\tTag\tpara#Shard/Para`
  (`main.go:253`): `Shard` уже часть составного поля, в этом же стиле
  можно расширить строку идентификатором словаря — но сейчас такого
  расширения нет.
- Ни в коде, ни в тестах нет ни одного примера «открыть N словарей и
  смёржить результаты». Единственное упоминание — `docs/requirements.md:165`,
  строка в таблице **исходного** (дореализационного) плана
  ("cmd/gomorphy — Адаптируется — CLI: ... multi-dict") — это
  нереализованный пункт старого roadmap, не факт текущего кода.
- Поле `Shard` в `Reading`/`LemmaRef` спроектировано расширяемо именно
  с прицелом на будущую мультисловарность (см.
  [superpowers/specs/2026-09-14-suffix-sharding-design.md](superpowers/specs/2026-09-14-suffix-sharding-design.md)) —
  то есть архитектурно продумано, но не реализовано.

**Открытые вопросы — решены 2026-09-16, см.
[docs/superpowers/specs/2026-09-16-multi-dict-design.md](superpowers/specs/2026-09-16-multi-dict-design.md)
и [docs/superpowers/plans/2026-09-16-multi-dict.md](superpowers/plans/2026-09-16-multi-dict.md):**
...
4. CLI — **вне охвата этого захода**, как и плотный алфавит для
   pymorphy2 (см. ниже). `cmd/gomorphy` остаётся с одним `-dict`.
5. Связь с 2-байтовым DAWG-алфавитом — соотношение зафиксировано:
   ...
   экспериментальной библиотечной возможностью без read-path/CLI (см.
   ниже) — не решаем одну и ту же задачу дважды.

### Плотный алфавит в проде: тезисы обсуждения — ЧАСТИЧНО РЕАЛИЗОВАНО

Обсуждалось 2026-09-15/16, сразу после мерджа ветки `dawg-alphabet-harness`
в master; пользователь взял паузу подумать. 2026-09-16 главный открытый
вопрос (см. «Обсуждали, но НЕ приняли решение» ниже) решён и реализован —
вариант A (pymorphy2-рекомпилятор), только для `words.dawg`, до рабочего
`Parse()`. Подробности реализации — в новом разделе ниже, сразу после
исторической записи обсуждения. Остальные тезисы (2-байтовый алфавит,
`.dat`-сериализация, `fuzzy.go`, `Prediction`/`Probability`) остаются
нереализованными design-заметками — раздел ниже сохранён как есть для
контекста, что было согласовано и почему.

**Что уже установлено и смёржено (не под вопросом):**
- Плотный 1-байтовый DAWG-алфавит: −36.2% размера `words.dawg` и втрое
  ...
  [docs/research/0004-dawg-dense-alphabet-with-payload.md](research/0004-dawg-dense-alphabet-with-payload.md).
- `Alphabet`/`IdentityAlphabet`/`DenseAlphabet` уже в
  `pkg/morphology/internal/alphabet.go` (смёржено) — но это пока
  измерительный harness, ничего не подключено к `import.go`/`open.go`/
  `save.go`/`parse.go`/`fuzzy.go`/формату `.dat`.

**Согласованные решения по дизайну продакшн-внедрения (обсуждены в
диалоге, НЕ формализованы в спеку, можно брать за основу при
продолжении):**
1. Таблица алфавита — общая на весь словарь (как `prefixes`), не
   своя у каждого шарда.
2. CLI (`gomorphy_build`/`gomorphy`) — без новых алфавитных флагов
   вообще. Всегда: плотный 1-байтовый алфавит, русский порядок по
   умолчанию, режим extend (см. п.4). Только уже запланированные
   базовые ключи `-i`/`-o`/`-t` (Этап 21), без добавок под алфавит.
3. Библиотека (Go API, встраивание) — вся гибкость: свой базовый
   алфавит (порядок, задаётся напрямую как `[]rune` в коде, не файлом
   — идея с текстовым файлом для CLI отменена), выбор ширины 1 или 2
   байта, выбор режима extend/strict.
...
8. Многоязычность — через параллельные словари (каждый свой файл, свой
   алфавит), не через один словарь с широким алфавитом. Это отдельная
   фича multi-dict (см. раздел выше), не трогалась в этом обсуждении.

**Найденная сложность read-path (главная причина взять паузу):**
...
2. `d.Suffixes`/`d.Prefixes` — вычисляются из **тех же** переменных
   ...
   сейчас это независимые пути. Если закодировать DAWG-ключ, а таблицы
   ...
   см. выше) — при реализации нужен явный regression-тест именно на
   эту синхронизацию.
3. `fuzzy.go` — весь аппарат нечёткого поиска (Левенштейн по рунам,
   ...
   Проверено чтением кода (эта же сессия), файлы:строки — в истории
   диалога, не задокументированы отдельно; при продолжении стоит
   перепроверить `pkg/morphology/parse.go`, `pkg/morphology/fuzzy.go`,
   `pkg/morphology/internal/similar_items.go`,
   `pkg/morphology/internal/char_policy.go` заново.
...
**Обсуждали, но НЕ приняли решение — главный открытый вопрос:**
предложение пользователя — выпилить прямую поддержку сырого pymorphy2
`words.dawg` (сейчас `pkg/morphology/importers/pymorphy2/import.go`
просто алиасит готовые бинарные артефакты pymorphy2 as-is, без единой
трансформации — `d.Words = []*internal.DAWG{words}`) и вместо этого
пересобирать pymorphy2-словари через тот же конвейер, что и
OpenCorpora (пройти исходный DAWG через `SimilarItems`/`completer`,
извлечь по каждому слову (текст, парадигма, форма), пересобрать с
плотным алфавитом). Тогда вся библиотека внутри всегда 1 байт, ни
одной развилки в read-path вообще.
- Оценка (эта сессия): технически осуществимо (примитивы чтения уже
  есть — `SimilarItems`/`completer` умеют читать сырой UTF-8 DAWG), но
  это реально новый объёмный кусок работы (полноценный
  «pymorphy2→gomorphy-native рекомпилятор»), больше по объёму, чем
  сама адаптация read-path под 2 формы (см. выше).
- Предложенная более дешёвая альтернатива: оставить pymorphy2-путь как
  есть (прямой alias, сырой UTF-8), read-path поддерживает ровно 2
  простые формы (переменная UTF-8 для pymorphy2/fallback, фикс-1-байт
  для словарей, которые строит сам gomorphy), а полный
  pymorphy2-рекомпилятор — отдельная будущая задача.
- **Обновление 2026-09-16** — оценка трудоёмкости шага «прочитать весь
  `words.dawg`» перепроверена и оказалась завышенной: полный обход
  реального pymorphy2-словаря (3 064 708 пар словоформа→(para,form))
  через уже существующие экспортированные `DAWG.ForEachChild`+
  `DAWG.ValuesForIndex` занял 570 мс на ~35 строках кода — не требует
  новой инфраструктуры и не трогает `Paradigms`/`Suffixes`/`Prefixes`
  (они у pymorphy2 не пересчитываются, в отличие от OpenCorpora-пути).
  Подробности и цифры:
  [docs/research/0005-pymorphy2-full-dawg-walk-cost.md](research/0005-pymorphy2-full-dawg-walk-cost.md).
- **Решено и реализовано 2026-09-16** — выбран вариант A: pymorphy2-рекомпилятор,
  но только для `words.dawg` (не `Prediction`/`Probability` — остаются
  неисследованными, отдельная будущая задача). Подробности — в разделе
  «pymorphy2-рекомпилятор под плотный алфавит» ниже.

### pymorphy2-рекомпилятор под плотный алфавит (words.dawg) — РЕАЛИЗОВАНО 2026-09-16

Дизайн: [docs/superpowers/specs/2026-09-16-pymorphy2-dense-recompile-design.md](superpowers/specs/2026-09-16-pymorphy2-dense-recompile-design.md).
План: [docs/superpowers/plans/2026-09-16-pymorphy2-dense-recompile.md](superpowers/plans/2026-09-16-pymorphy2-dense-recompile.md)
(6 задач, subagent-driven-development, финальное ревью + 1 fix-волна).

Что появилось:
- `internal.DAWG.Walk(fn func(key string, values [][]byte))` — общий
  примитив полного обхода DAWG (не завязан на pymorphy2; пригодится и
  для будущего multi-dict, см. выше).
...
- **Явные ограждения от тихой порчи данных** (найдено финальным ревью
  всей ветки, тот же класс риска, что уже дважды был в проекте — см.
  «Критический баг: искажённые теги...» выше): `SaveTo` теперь
...
Осознанно НЕ сделано в этом заходе (следующие шаги, если понадобятся):
- Сериализация `Alphabet` в `.dat` и поддержка в `Open()` — п.5 из
  согласованных решений выше остаётся нереализованным; поэтому
  `SaveTo` сейчас просто отказывает, а не поддерживает.
- `fuzzy.go` — по-прежнему предполагает UTF-8-байты DAWG; для плотных
  словарей отключён (возвращает `nil`), не переосмыслен.
- `Prediction`/`Probability` DAWG — не исследованы (другой формат
  ключей: суффиксы слов и `"слово:тег"` с ASCII-граммемами).
- CLI (`gomorphy_build`)/Этап 21 — не тронуты, это Go-API-only инкремент.

### Получение словарей pymorphy2 (download+unpack) — РЕАЛИЗОВАНО

Реализован узкий срез плана из «Этап 21» ниже — только шаги `download`
и `unpack` (без `compile`, без редизайна CLI `gomorphy_build`), нужный
как источник реальных данных для находки выше. Новый пакет
`pkg/pymorphy` (`const.go`, `errors.go`, `pypi.go`, `loader.go`) по
образцу `pkg/opencorpora`: `Loader.Sync(skipDownload)` = download+unpack;
`DownloadUpdate()` — PyPI JSON API → wheel → `.data/pymorphy/pymorphy2-dicts-ru.whl`
+ `version.txt`; `UnpackUpdate()` — распаковка ZIP-поддерева
`pymorphy2_dicts_ru/data/*` → `.data/pymorphy/data/`. Один сознательный
отход от буквального текста плана «Этап 21»: имя скачанного файла
фиксировано (`pymorphy2-dicts-ru.whl`, без версии в имени), версия PyPI
хранится отдельно в `version.txt` — проще, чем парсить версию из имени
файла на каждой проверке обновления. `compile pymorphy` (сборка в единый
`.dat`) и редизайн CLI — по-прежнему не сделаны, это остаётся частью
Этапа 21 целиком.
...
Обе критические находки код-ревью перед 1.0.0 закрыты. Груминг CLI
также закрыт — см. Этап 21 ниже; остаётся сам Этап 18 (документация,
тесты, метрики).

### Этап 18. Финализация: документация, тесты — НЕ НАЧАТО

> CLI-редизайн (`cmd/gomorphy`, ранее также `cmd/gomorphy_build`) уже
> выполнен отдельно — см. Этап 21. Этот этап — оставшаяся документация,
> тесты, ожидаемые метрики.
...
## Этап 21. CLI-редизайн gomorphy_build + источник pymorphy2 — РЕШЕНО И РЕАЛИЗОВАНО 2026-09-16 (поглощено CLI-редизайном)

**Обновление 2026-09-16**: этот этап поглощён более широким редизайном —
вместо переформатирования `cmd/gomorphy_build` в отдельный конвейер оба
бинаря (`cmd/gomorphy`, `cmd/gomorphy_build`) заменены одним cobra-based
`gomorphy` с командами `lookup`/`lemmas`/`fuzzy`/`top`/`cli` и
`download`/`unpack`/`build`/`update`. См.
`docs/superpowers/specs/2026-09-16-cli-redesign-design.md` и
`docs/superpowers/plans/2026-09-16-cli-redesign.md`. Обсуждение ниже
сохранено как контекст решения (источник pymorphy2, конвейер по типам
словарей и т.д.) — конкретный интерфейс (`gomorphy_build <command>
<dict_type>`) заменён на описанный в новом плане/спеке.
...
Поднято пользователем 2026-09-16 при проектировании multi-dict: даже
сейчас, без всякого multi-dict, разные словари уже описывают граммемы
по-разному, хотя формально это один и тот же набор. Конкретный,
проверенный чтением кода пример: pymorphy2 (`.data/pymorphy/data/gramtab-opencorpora-int.json`)
использует `"NOUN,anim,masc sing,nomn"` (пробел перед `sing`), а
OpenCorpora-импортёр (`pkg/morphology/importers/opencorpora/import.go`)
всегда собирает тег через запятую без пробелов — то есть один и тот же
смысл, разные строки. `TagSet.Name` (`"opencorpora"` vs
`"opencorpora-int"`) уже фиксирует эту разницу конвенций как метаданные,
но никак её не устраняет. Отдельно в Этапе 19 (тематические словари,
TSV-импорт) уже заложены **opaque-теги** — произвольные строки без
всякого маппинга на OpenCorpora-набор — то есть будущие словари могут
разойтись не только синтаксисом, но и словарём граммем целиком.

Выбран путь (пользователь, 2026-09-16): **общая расширяемая архитектура**
маппинга «свой тег словаря → универсальный тег», а не точечная
нормализация синтаксиса opencorpora/pymorphy2. Не спроектирована —
нужен отдельный brainstorming перед стартом, вероятно отдельная спека.
Открытые вопросы (не решены):
- Где живёт таблица соответствий — часть `TagSet` (по аналогии с
  `Name`), отдельная структура рядом, или конфигурация на уровне
  библиотеки/CLI?
- Как обрабатывать opaque-теги тематических словарей (Этап 19) — они
  по определению не имеют канонического соответствия; нужен ли
  fallback («тег как есть», без маппинга) вместо ошибки?
- Формат «универсального тега» — свой ли это независимый набор
  (аналог UniMorph feature-value пар?), или сведение к одному из
  существующих наборов (например, canonical OpenCorpora)?
- Связь с multi-dict (см. раздел выше) — `MultiDictionary.Parse()`
