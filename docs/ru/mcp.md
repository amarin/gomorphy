...
случае *тонкий слой поверх `pkg/morphology`*, который вместо CLI-вызовов даёт
...
- JSON-схемы инструментов и маппинг ошибок (ошибки библиотеки и пустые
  результаты → структурированные ответы);
...
ноль зависимостей и переиспользование существующих `Parse`/`Lemma`/`Fuzzy`.
...
- единый источник истины — `pkg/morphology` / импортёры, MCP только
  прокидывает вызовы.

До появления такого потребителя: сознательно минимальный CLI (импорт уже
есть, батч-запросы запланированы) + запланированные скиллы «использование
словаря gomorphy» (этап 18) и «тематический словарь» (этап 19) — оба ещё не
начаты, см. [todo.md](../en/todo.md) — единственный интерфейс агента к gomorphy.