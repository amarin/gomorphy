...
- ОС: Linux, macOS (amd64/arm64); сборка под Windows компилируется, но
  `Open` возвращает ошибку во время выполнения — загрузка словаря
  использует mmap через пакет `syscall` напрямую (см.
  `docs/code-review-pre-1.0.md`). `OpenBytes` работает на Windows (без
  mmap) — это текущий обходной путь для загрузки словаря там (например,
  встроить `.dat`-файл через `//go:embed` и передать его байты) — см.
  `docs/en/todo.md`, «Windows: native mmap»
