...
Длина строки ограничена 1 МиБ. В отличие от `Builder.Build`, пустой поток
или поток из одних комментариев — не ошибка: возвращается пустой словарь
(любой поиск промахивается) и `nil`-ошибка. Вариант для файла не
