...
- **`OpenBytes(data)`** — тот же формат, что `Open`, но из памяти
  (например, `//go:embed`), без mmap — работает и на Windows; `data`
  не копируется и должен жить дольше словаря, `Close()` для такого
  словаря — no-op.
...
(`OpenPyMorphy*`/`CompileFromXML*`/`OpenBytes` не используют mmap),
вызывать можно безусловно. `Close` нельзя вызывать, пока другие
горутины работают со словарём (напрямую или через `MultiDictionary`):
незавершённый вызов `Parse`/`Lemma`/`IsKnown`/`Fuzzy`/`FuzzyTop`/
`ContentHash` читает отображённую память, и снятие отображения под ним
роняет процесс (SIGSEGV/SIGBUS, не перехватываемая паника); возвращённые
строки остаются валидными и после `Close`.
...
- `BuilderOptions{Language, Source, CharPolicy}` — `Language` используется
  ...
  умолчанию `"builder"`).
- `AddForm`/`AddLemma` и `ImportTSV` приводят слово и лемму к нижнему
  регистру (`strings.ToLower`) — раньше форма, добавленная как «Москва»,
  находилась только предсказанием, неотличимым от словарного разбора.
- `CharPolicy: nil` (по умолчанию) выбирает политику по `Language`: е→ё
  только для `"ru"` и для пустого `Language` (= «ru»), для остальных
  языков — без замен; `NoCharPolicy()` отключает замены явно, а
  `NewCharPolicy(Substitution{...})` задаёт свою политику.
...
## Словарные разборы и предсказания

`Parse` при отсутствии слова в словаре подставляет предсказание по
окончанию — такой разбор ничем не отличается от обычного, кроме
`Predicted == true` (у `LemmaRef.Predicted` — `true`, если все разборы
за леммой предсказаны). Чтобы проверить принадлежность слова словарю без
предсказания, используйте `IsKnown`:
...
`IsKnown(word)` приводит `word` к нижнему регистру и применяет
`CharPolicy` — как и `Parse` — и никогда не предсказывает.
`MultiDictionary.IsKnown` — `true`, если слово знает хотя бы один
словарь набора.
...
`ContentHash()` возвращает стабильный hex-дайджест (xxh3-128) содержимого
словаря без секции `info`: пересохранение без изменений или открытие
через `Open`/`OpenBytes` дают тот же дайджест; результат первого вызова
кешируется.
