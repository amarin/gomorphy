...
**Файл**: `pkg/morphology/importers/opencorpora/import.go:225-235` (`xmlHandler.OnForm`)
...
- **`pkg/morphology/internal/tagset.go:Add` (16-27)** и
  **`pkg/morphology/importers/opencorpora/import.go:127,140`**
  ...
  Сегодня с запасом (реально ~1K/5K/3K), но для будущих источников
  (Этап 19 — тематические словари с произвольными пользовательскими
  тегами) стоит явная проверка с понятной ошибкой вместо тихого
  переполнения.
