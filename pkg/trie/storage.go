...
func (graph *Trie[A, M]) StorageConfig() storage.Config {
...
func (graph *Trie[A, M]) ReadFrom(r io.Reader) (n int64, err error) {
	graph.mu.Lock()
...
func (graph *Trie[A, M]) WriteTo(w io.Writer) (n int64, err error) {
	graph.mu.Lock()
	defer graph.mu.Unlock()
